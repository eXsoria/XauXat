        ...
        VStack {
            List {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("Share your XauXat address on social media.")
                    ...
                    if !showCreateAddressButton {
                        (Text(Image(systemName: "shield")).foregroundColor(.secondary) + textSpace + Text("Connection security").font(.title2).bold())
                            .padding(.top)
                        Text("XauXat addresses and one-time links are safe to share via any messenger.")
